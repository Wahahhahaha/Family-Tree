...
@section('content')
<?php
    $customFields = \DB::table('custom_fields')->whereNull('deleted_at')->get();
...
<div class="wrapper">
    <?php
        ...
        $firstShowActionBlock = $firstCanDeleteUser || $firstCanDeleteChild || $firstCanUpdateLifeStatus || $firstCanEditProfile || $firstCanDivorcePartner;
        if ($isSuperadmin) {
        ...
        <aside id="detailSidebar" class="detail detail-shell hidden">
            ...
            <div id="memberDetailBlock" class="hidden <?php echo e($activePanel === 'add-member' ? 'hidden' : ''); ?>">
                ...
                <div
                    ...
                    data-custom-fields='<?php echo e($firstMember->custom_fields ?? "{}"); ?>'
                >
                    ...
                    <div id="detailViewBlock">
                        <ul class="detail-list">
                            ...
                            <div id="detailCustomFieldsWrap">
                                <?php 
                                    $cFields = json_decode($firstMember->custom_fields ?? "{}", true);
                                    if(is_array($cFields) && !empty($cFields)):
                                        foreach($cFields as $fId => $fVal):
                                            $fName = $customFields->firstWhere("id", $fId)->field_name ?? "Field ".$fId;
                                            if(!empty($fVal)):
                                ?>
                                    <li><span><?php echo e($fName); ?></span><strong><?php echo e($fVal); ?></strong></li>
                                <?php 
                                            endif;
                                        endforeach;
                                    endif;
            ...
            <?php if ($currentLevelId === 2 || $isSuperadmin || $canDeleteAnyUserFromHome): ?>
                <div id="memberActionBlock" class="member-action-block hidden <?php echo e($firstShowActionBlock ? '' : 'hidden'); ?>">
                        ...
                        <form id="divorcePartnerForm" method="POST" action="/family/member/divorce" class="<?php echo e($firstCanDivorcePartner ? '' : 'hidden'); ?>" data-delete-message="Divorce this partner relationship?">
                            <?php echo csrf_field(); ?>
                        ...
                        <?php if ($canDeleteAnyUserFromHome): ?>
                            <form id="deleteUserForm" method="POST" action="/management/users/<?php echo e((int) ($firstMember->userid ?? 0)); ?>/delete" class="<?php echo e($firstCanDeleteUser ? '' : 'hidden'); ?>" data-delete-message="Move this user to the recycle bin?">
                                <?php echo csrf_field(); ?>
...
<script>
window.masterCustomFields = <?php echo json_encode($customFields ?? []); ?>;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showMemberDetailFromCard(card) {
        ...
        console.log('Clicked Member ID:', memberId, 'Custom Fields:', customFieldsJson);
        ...
        if (canDeleteAnyUserFromHome) {
            if (deleteUserForm) {
                var shouldShowDeleteUser = userId !== '' && String(userId) !== String(currentUserId);
    ...
    if (detailEditPictureInput) {
        detailEditPictureInput.addEventListener('change', function () {
            ...
            setDetailPhotoPreviewFromFile(file);
        });
    }
